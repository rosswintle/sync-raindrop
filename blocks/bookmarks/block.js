...
function createSyncRaindropList(props) {
...
async function fetchBookmarks (startDate, endDate, props) {
	// TODO: localise this
	let response = await fetch(`/wp-json/wp/v2/raindrop-bookmark/?after=${startDate}&before=${endDate}&per_page=100`);
...
registerBlockType( 'sync-raindrop/bookmarks', {
	title: 'Bookmarks',
	icon: 'post-status',
	category: 'common',
	attributes: {
	...
	edit: function( props ) {
		...
		return [
			...
			createSyncRaindropList(props)
		];
	},
	save: function( props ) {
		const list = createSyncRaindropList(props);
		return el(
			'div',
			{ style: blockStyle },
			list
